...
function rfc7231Date(d = new Date()) {
    return d.toUTCString();
}
function sha256Base64(s) {
    return crypto.createHash("sha256").update(s).digest("base64");
}
export function signHttpRequest(opts) {
    ...
    if (method === "post") {
        if (opts.body === undefined)
            throw new Error("signHttpRequest: POST requires body");
    ...
    const signature = `keyId="${opts.keyId}",` +
        `algorithm="rsa-sha256",` +
        `headers="${signedHeaders.join(" ")}",` +
        `signature="${sig}"`;
    const out = { host, date, signature };
    if (digest) {
        out.digest = digest;
...
export async function verifyHttpSignature(input) {
    ...
    if (!parts.keyId)
        return { ok: false, reason: "Signature: keyId missing" };
    if (!parts.headers)
        return { ok: false, reason: "Signature: headers missing" };
    if (!parts.signature)
        return { ok: false, reason: "Signature: signature missing" };
    ...
    if (Number.isNaN(dateMs))
        return { ok: false, reason: "unparseable Date header" };
    ...
    if (signed.includes("digest")) {
        ...
        if (!digestHdr)
            return { ok: false, reason: "Digest header missing but signed" };
    ...
    const block = signed
        .map((h) => {
        if (h === "(request-target)")
            return `(request-target): ${input.method.toLowerCase()} ${path}`;
    ...
    if (!pubKeyPem)
        return { ok: false, reason: "could not fetch public key" };
    let valid = false;
    try {
        valid = crypto
...
//# sourceMappingURL=httpsig.js.map